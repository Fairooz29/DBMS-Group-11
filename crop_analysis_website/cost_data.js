document.getElementById('add-row-btn').addEventListener('click', function () {
    ...
    newRow.innerHTML = `
        ...
        <td><input type="number" placeholder="Cost (in USD)"></td>
        <td>
    ...
    newRow.querySelector('.save-btn').addEventListener('click', function () {
        ...
        const cost = newRow.cells[2].querySelector('input').value;

        if (year && crop && cost) {
            newRow.cells[0].innerHTML = year;
            newRow.cells[1].innerHTML = crop;
            newRow.cells[2].innerHTML = cost;
            ...
            newRow.querySelector('.edit-btn').addEventListener('click', function () {
                ...
                newRow.cells[2].innerHTML = `<input type="number" value="${cost}">`;
                newRow.cells[3].innerHTML = `
                    <button class="save-btn">Save</button>
                    <button class="delete-btn">Delete</button>
                `;
                ...
                newRow.querySelector('.save-btn').addEventListener('click', function () {
                    ...
                    const updatedCost = newRow.cells[2].querySelector('input').value;

                    if (updatedYear && updatedCrop && updatedCost) {
                        newRow.cells[0].innerHTML = updatedYear;
                        newRow.cells[1].innerHTML = updatedCrop;
                        newRow.cells[2].innerHTML = updatedCost;
                        newRow.cells[3].innerHTML = `
                            <button class="edit-btn">Edit</button>
                            <button class="delete-btn">Delete</button>
                        `;
